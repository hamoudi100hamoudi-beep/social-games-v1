...
import { TOP_COLORS, BOT_COLORS } from '../utils/drawBinaryHelper';
import DrawingCanvasCore, { DrawingCanvasCoreHandle } from './game/DrawingCanvasCore';
...
export default function DrawingBoard({ 
  ...
  const canvasCoreRef = useRef<DrawingCanvasCoreHandle>(null);
  ...
  // Colors Palette Horizontal Scroll Handler (Touch-pan-x and Mouse-drag)
  useEffect(() => {
  ...
  const confirmClear = () => {
    canvasCoreRef.current?.clearCanvas(true);
      ...
      <div ref={containerRef} dir="ltr" className="flex-1 relative bg-slate-100 overflow-hidden w-full h-full cursor-crosshair">
        ...
        {!readOnly && (
          <div className="absolute top-3 left-3 flex gap-1.5 z-30 pointer-events-auto">
            <button 
              type="button"
              onClick={() => canvasCoreRef.current?.undo()} 
            ...
            <button 
              type="button"
              onClick={() => canvasCoreRef.current?.redo()} 
