...
- [x] **M4.6:** نظام الخمول الكسول ومستشعرات الرؤية الذكية للغلق الهادئ والآمن، بالإضافة إلى إعادة تصميم صفحات "الخمول" (Idle Popup) و"طرد الخمول" (Inactive Disconnection Overlay) بالأسلوب السينمائي العالي الكرتوني (Style 2) لتطابق الهوية المرئية وتظهر ممركزة وبشفافية متناسقة فوق اللعبة واللوبي.
...
- [x] **تصفية اهتزاز الإحداثيات (Sub-Pixel Jitter Debounce Filtering):** حظر الارتجاجات المجهرية وسد الثغرات الرسومية للمستشعرات اللمسية الضعيفة عن طريق تصفية الفروق الهندسية الدقيقة (عبر تصفية النقاط المتقاربة بأقل من 4 بكسل في المستوى الثالث و2 بكسل في المستوى الثاني). هذه الخطوة قللت من عدد النقاط المرسلة والمصيرة بنسبة تزيد عن 65% مما حرر المعالج وحوّل رسم المنحنيات لسرعة فائقة واستجابة ناعمة حتى عند السحب السريع بالإصبع.
- [x] **توطيد طبقات المعالجة وتكامل الـ Canvas:** تفادي الشاشات السوداء التي ظهرت على الهواتف الحديثة نتيجة الخاصية التجريبية لعدم التزامنية (`desynchronized: true`) بإزالتها كلياً، واستبدالها بتوجيه المعالجة الرسومية للمتصفح عبر الخياطة السريعة للـ CSS باستخدام تلميحات الأولوية العالية `willChange: 'transform'`.
- [x] **تطوير تطبيق ويب تقدمي قابل للتثبيت كلياً (Progressive Web App - PWA):** تحويل اللعبة إلى تطبيق مستقل قابل للمحاكاة الكاملة والتثبيت الفوري من المتصفح لمعالجة الشريط العلوي للمتصفح والحصول على مساحة شاشة قصوى (Full-Screen Immersive Canvas):
  - **مظهر مستقل وبدون هوامش (Immersive Standalone Display):** تكوين ملف المانيفست `manifest.json` بإطلاق وضعية `standalone` وتفعيل التخزين المتناسق للمتصفح لإعطاء الإحساس بتطبيق أصيل وخارجي.
  - **أيقونة مشغل فخمة وعالية الدقة (PWA Squircle Launcher Icon):** تصميم وبناء أيقونة ترحيب عصرية ذكية بأبعاد دقيقة وتوزيع مذهل لألوان الطيف والأقلام المشعة لتظهر كأيقونة حقيقية على شاشات الهواتف.
  - **خادم خدمات ذكي مركب (Service Worker):** تطوير خادم `sw.js` يعمل بمنهجية "Stale-While-Revalidate" لتخزين الملفات الهيكلية للموقع وضمان تحميل فوري، مع عزل قنوات وتدفقات الـ Websockets ولقاءات الـ API تماماً لعدم إعاقة سرعة اللعب في الغرف.
